...
void f(System* sys) {
	...
	//Process* klon2 = sys->cloneProcess(pid);
	assert(klon != nullptr);
	ProcessId klonId = klon->getProcessId();
	//ProcessId klon2Id = klon2->getProcessId();
#endif
...
#ifdef CLONE
			rotate(sys, klon, klonId, vaddr, rep);
			//rotate(sys, klon2, klon2Id, vaddr, rep);
#endif
		}
	...
	/*for (VirtualAddress vaddr = 0UL; vaddr < PAGE_SIZE * SEG_SIZE; ++vaddr) {
		GLOBAL_LOCK.lock();
		Status status = sys->access(klon2Id, vaddr, READ);
		assert(status != Status::TRAP);
		if (status == Status::PAGE_FAULT)
			klon2->pageFault(vaddr);
		char* paddr = (char*)klon2->getPhysicalAddress(vaddr);
		///
		char* now = paddr;
		char* before = (char*)starting_content + vaddr;
		rotate(before, REP);
